...
const GamePlayerDetails = () => {
  const dispatch = useDispatch();
  const navigate = useNavigate();
  const { state } = useLocation();

  const { gameItem } = state;
  const [roomDetails, setRoomDetails] = useState({});
  const [showDeclare, setShowDeclare] = useState(false);
  const [teamId, setTeamId] = useState();
  const { gamePlayersDetails, loading } = useSelector(
    (state) => state.gameReducer
  );
  const [playerListDetails, setPlayerListDetails] = useState({});
  const playerDetails = playerListDetails?.player;
  const competitorDetails = gameItem?.competitor;
  useEffect(() => {
    if (gamePlayersDetails !== undefined) {
      setPlayerListDetails(gamePlayersDetails);
    }
  }, [gamePlayersDetails]);
  ...
  const overview = [
    {
      name: "Name",
      value: gameItem?.game?.name,
    },
    {
      name: "Game Status",
      value: gameItem?.gameStatus,
    },
    {
      name: "Bet Amount",
      value: `₹ ${gameItem?.betAmount}`,
    },
    {
      name: "Request Type",
      value: gameItem?.requestType,
    },
    // {
    //   name: "Mode",
  ...
  useEffect(() => {
    dispatch(gamePlayersTdmDetails(state?.query));
        ...
        <div className="w-full relative table-container overflow-y-scroll bg-secondary rounded shadow">
          ...
          <section className="relative rounded-t-lg block h-44 sm:h-52 md:h-60 lg:h-72">
            <div
              ...
              style={{
                backgroundImage: `url("${gameItem?.game?.gameImage}")`,
              }}
            >
              <span
                id="blackOverlay"
...
export default Layout(GamePlayerDetails);
