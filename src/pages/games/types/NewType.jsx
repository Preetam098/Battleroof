...
import { addGameType, updateGameType } from "../../../redux/actions/gameAction";
import ShowError from "../../../components/ShowError";

const NewType = ({ handleCloseModal, imageUrl, editData }) => {
  ...
  useEffect(() => {
    const fetchImage = async () => {
      try {
        const response = await fetch(`${imageUrl}${editData?.image}`);
  ...
  const handleSubmit = (event) => {
    ...
    if (validator.allValid()) {
      if (editData) {
        if (isEdit) {
          dispatch(updateGameType(editData._id, payload, handleCloseModal));
        } else {
          handleCloseModal();
        }
      } else {
        dispatch(addGameType(payload, handleCloseModal));
      }
    } else {
